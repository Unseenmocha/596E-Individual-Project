...
class NER_Processing:
    def __init__(self):
        ...
        self.id2label = {0: "LABEL_0", 1: "O", 2: "B-MISC", 3: "I-MISC", 4: "B-PER", 5: "I-PER", 6: "B-ORG",
        7: "I-ORG", 8: "B-LOC", 9: "I-LOC", 10: "[CLS]", 11: "[SEP]"}

        self.session = onnxruntime.InferenceSession("NER.onnx", providers=["CPUExecutionProvider"])
    
    def preprocess(self, input):
        inputs = self.tokenizer(input, return_tensors="pt", truncation=True, is_split_into_words=False)
    ...
    def postprocess(self, output, input_ids):
        ...
        for token, label in zip(words, predicted_labels):
            if token.startswith("##"):
                ...
                if current_word and current_label not in ['O','[SEP]', '[CLS]']:  # Store the previous word and its label
    ...
    def predict(self, input):
        ...
        output = self.session.run(['output'], {'input_ids': input_ids, 'attention_mask': inputs["attention_mask"]})
